 ...
 * per-test accuracy breakdown (correct / wrong / unattempted).
 */
            ...
            <div className="flex items-end gap-3 h-52" role="img" aria-label={`Bar chart of all ${chartData.length} test scores in order`}>
              {chartData.map((r) => (
                <div key={r.resultId} className="flex-1 flex flex-col items-center gap-2 min-w-0">
                  ...
                  <div
                    className={`w-full max-w-14 rounded-t-lg ${r.status === "PASS" ? "bg-[var(--royal)]" : "bg-red-300"}`}
          ...
          <div className="card p-6">
            ...
            <div className="space-y-5">
              {results.map((r) => {
                ...
                const unattemptedPct = (r.unattempted / total) * 100;
                return (
                  <div key={r.resultId}>
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <p className="text-[13.5px] font-medium">{r.examName || "Test (removed)"}</p>
                      <p className="text-[11.5px] text-[var(--ink-soft)]">
                        {r.correct} correct · {r.wrong} wrong · {r.unattempted} unattempted
                      </p>
                    </div>
                    <div className="flex h-2.5 rounded-full overflow-hidden bg-[var(--bg-soft)]">
                      ...
                      <div className="bg-gray-300" style={{ width: `${unattemptedPct}%` }} />
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex items-center gap-4 mt-6 text-[11.5px] text-[var(--ink-soft)]">
              ...
              <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-300 inline-block" /> Unattempted</span>
            </div>
          </div>
        </>
      )}
